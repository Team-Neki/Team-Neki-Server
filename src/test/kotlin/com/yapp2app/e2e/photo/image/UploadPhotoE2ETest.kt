...
class UploadPhotoE2ETest : PhotoImageE2ETestBase() {
    ...
    fun givenValidMediaId_whenUploadPhoto_thenReturnsSuccess() {
        ...
        RestAssured.given()
            ...
            .body(UploadPhotoRequest(media.id, null, null))
            .`when`()
            .post("/api/photos")
            .then()
    ...
    fun givenValidMediaIdAndFolderId_whenUploadPhoto_thenReturnsSuccess() {
        ...
        RestAssured.given()
            .contentType(ContentType.JSON)
            .header("Authorization", "Bearer $accessToken")
            .body(UploadPhotoRequest(media.id, folder.id, null))
            .`when`()
            .post("/api/photos")
            .then()
            .statusCode(HttpStatus.OK.value())
    ...
    fun givenMissingMediaId_whenUploadPhoto_thenReturnsBadRequest() {
        ...
        RestAssured.given()
            .contentType(ContentType.JSON)
            .header("Authorization", "Bearer $accessToken")
            .body(UploadPhotoRequest(null, null, null))
            .`when`()
            .post("/api/photos")
            .then()
    ...
    fun givenNonExistentMediaId_whenUploadPhoto_thenReturnsNotFound() {
        ...
        RestAssured.given()
            .contentType(ContentType.JSON)
            .header("Authorization", "Bearer $accessToken")
            .body(UploadPhotoRequest(999999, null, null))
            .`when`()
            .post("/api/photos")
            .then()
            .statusCode(HttpStatus.BAD_REQUEST.value())
