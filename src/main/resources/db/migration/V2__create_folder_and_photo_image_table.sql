...
COMMENT
ON COLUMN TB_PHOTO_IMAGE.file_id IS '파일 고유 ID (S3 키 등)';
COMMENT
ON COLUMN TB_PHOTO_IMAGE.folder_id IS '폴더 ID (nullable, 폴더 삭제 시 NULL)';
COMMENT
ON COLUMN TB_PHOTO_IMAGE.status IS '업로드 상태 (INITIATED, UPLOADED, VERIFIED, FAILED)';
COMMENT
...
COMMENT
ON COLUMN TB_MEDIA.media_type IS '미디어 타입 (USER_PROFILE, PHOTO_BOOTH)';
COMMENT
