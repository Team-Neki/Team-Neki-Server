...
class KakaoRegisterUseCase(
     ...
     * 3. 검증 실패 시 캐시 무효화 후 재시도 (공개키 로테이션 대응)
     * 4. oauthInfoResult 값 여부에 따라 회원가입 처리
     *
     * OIDC 공개키 캐싱 전략 (클린 아키텍처):
     * - 1차 시도: 캐시된 공개키로 토큰 검증
     * - 검증 실패 시: RedisCacheService.evict()로 캐시 무효화 후 재검증
     ...
     */
    private fun validateTokenWithPublicKeys(idToken: String): OauthInfoResponse {
