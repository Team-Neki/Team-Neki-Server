...
class ArchitectureRulesTest {
    ...
    inner class DtoPlacement {
        ...
        // (예: com.neki.common.domain.vo.Pagination 이전의 PageRequest)
        // 공유 커널은 특정 계층의 DTO가 아니므로 검사 대상에서 제외한다.
        private val sharedKernel = "com.neki.common.domain.."
        ...
        // 유스케이스 입력(Command/Query)은 :domain 의 com.neki.<domain>.dto 로 이관이 끝났다.
        @Test
        fun `Command 클래스는 도메인 dto 패키지에만 위치해야 한다`() {
            classes()
                .that().haveSimpleNameEndingWith("Command")
                .and().resideOutsideOfPackage(sharedKernel)
                .should().resideInAnyPackage("com.neki.*.dto..")
