...
@UseCase
class CopyPhotosToFolderUseCase(private val folderService: FolderService, private val photoService: PhotoService) {

    @Transactional
    fun execute(command: FolderCommand.CopyPhotosToFolder) {
        folderService.validateFoldersOwned(command)
        photoService.validatePhotosOwned(command)

        folderService.addPhotosToFolders(command)
    }
}
